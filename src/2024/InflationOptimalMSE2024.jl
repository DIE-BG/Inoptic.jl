...
ENSEMBLE1 = [
    ...
    InflationCoreMaiF(GTDATA, [0.0, 0.006287041634702409, 0.4763169240418438, 0.8411085012613978, 1.0]), #<- Called MaiFP in priors versions of InflationFunctions
    InflationCoreMaiFG(GTDATA, [0.0, 0.30007700254937775, 0.3806979438933833, 0.6592801609973685, 0.8572920039185152, 1.0]), #<- Called MaiF in priors versions of InflationFunctions
    InflationCoreMaiG(GTDATA, [0.0, 0.2658236991960707, 0.5601153961330196, 0.8278967604925729, 1.0]),
]
...
ENSEMBLE2 = [
    ...
    InflationFixedExclusionCPI{3}(([35, 30, 190, 36, 37, 40, 31, 104, 162, 32, 33, 159, 193, 161], [29, 31, 116, 39, 46, 40, 30, 35, 186, 47, 197, 41, 22, 48, 185, 34, 184], [])),
    InflationCoreMaiF(GTDATA_CALIB_2024, [0.0, 0.5284098303517797, 0.5601390976116516, 0.8452089641556906, 0.9993493866637225, 1.0]), #<- Called MaiFP in priors versions of InflationFunctions
    InflationCoreMaiFG(GTDATA_CALIB_2024, [0.0, 0.31862161044309834, 0.4455906849926661, 0.8172109274523476, 1.0]), #<- Called MaiF in priors versions of InflationFunctions
    InflationCoreMaiG(GTDATA_CALIB_2024, [0.0, 0.42007669750748233, 0.5, 0.7913978925837042, 1.0]), #<- Called MaiG in priors versions of InflationFunctions
]
...
"""
inflation_optmse2024 <: CombinationFunction
Optimate MSE 2024  combination inflation function.
"""
InflationOptimalMSE2024 = InflationSpliceUnweighted(
    [
        c1,
        c2,
        c2,
    ];
    name = "Optimal MSE Linear Combination (2024 Edition)",
    tag = "InflationOptimalMSE2024"
)
